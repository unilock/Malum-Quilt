...
public interface MalumPlacedFeatureRegistry {
	...
	static void init(Registry<ConfiguredFeature<?, ?>> configured, DynamicRegistryManagerSetupContext.RegistryMap registryMap) {
		...
		registryMap.register(RegistryKeys.PLACED_FEATURE, Malum.id("runewood_tree"), new PlacedFeature(runewood, VegetationPlacedFeatures.treePlacementModifiers(RarityFilterPlacementModifier.create(40), MalumObjects.RUNEWOOD_SAPLING)));

		BiomeModifications.create(Malum.id("worldgen")).add(ModificationPhase.ADDITIONS, (b) -> b.isIn(BiomeTags.FOREST), context -> context.getGenerationSettings().addFeature(GenerationStep.Feature.TOP_LAYER_MODIFICATION, MalumPlacedFeatureRegistry.RUNEWOOD_CHECKED));
	}
