...
import static dev.sterner.malum.common.registry.MalumConfiguredFeatureRegistry.CONFIGURED_FEATURE_KEYS;
import static dev.sterner.malum.common.registry.MalumConfiguredFeatureRegistry.CONFIGURED_RUNEWOOD_TREE_FEATURE;
...
public interface MalumPlacedFeatureRegistry {
	...
	static void init(Registry<ConfiguredFeature<?, ?>> configured, DynamicRegistryManagerSetupContext.RegistryMap registryMap) {
		...
		BiomeModifications.create(Malum.id("worldgen")).add(ModificationPhase.ADDITIONS, (b) -> b.isIn(BiomeTags.FOREST), context -> context.getGenerationSettings()
