...
public class SpiritAltarBlock<T extends SpiritAltarBlockEntity> extends WaterLoggedEntityBlock<T>{
    ...
    public void scheduledTick(BlockState state, ServerWorld world, BlockPos pos, RandomGenerator random) {
        ((SpiritAltarBlockEntity) world.getBlockEntity(pos)).tick(state, world, pos, random);
    }
    ...
    public void onStateReplaced(BlockState state, World world, BlockPos pos, BlockState newState, boolean moved) {
        if (!state.isOf(newState.getBlock())) {
            ...
            if (blockEntity instanceof SpiritAltarBlockEntity) {
                ...
                ItemScatterer.spawn(world, pos, ((SpiritAltarBlockEntity) blockEntity).spiritInventory.getStacks());
    ...
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        return world.isClient ? checkType(type, MalumBlockEntityRegistry.SPIRIT_ALTAR, (world1, pos, state1, blockEntity) -> blockEntity.clientTick(world1, pos, state1)) : null;
    }
